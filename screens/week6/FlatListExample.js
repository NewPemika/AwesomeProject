...
// item แต่ละอันคือการแสดงผลคนละแบบ(format)
const Item = ({ item }) => (
...
const FlatListExample = () => {
  return (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      <FlatList
        data={posts}
        renderItem={({ item }) => <Item3 item={item} />} // เปลี่ยน item format  จะเปลี่ยน
        keyExtractor={item => item.id}
        numColumns={2} //เกิดปัญหา on the fly -> แก้โดยการดีดแอปทิ้งแล้วเปิดใหม่
        horizontal={false}
      />
      <Button title='Next' />
    </View>
